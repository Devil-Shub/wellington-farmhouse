...
<td class="text-xs-right">{{ props.item.insurance_expiry }}</td>
  ...
  <router-link :to="'/admin/skidsteer/addinsurance/' +vehicle_id" class="nav-item nav-link">Add Insurance</router-link> 
        </v-card>
      </v-tab-item>
...
components: {
  ...
  data() {
   ...
   mounted: function() {
        ...
        skidsteerService.getTruckInsurance(this.$route.params.id).then(response => {
              //handle response
              if(response.status) {
